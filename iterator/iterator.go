...
func New(synthesizer *synthesizer.Synthesizer, inspector *inspector.Inspector, tuner *tuner.Tuner, iterLimit int) *Iterator {
	p := new(Iterator)
	p.synthesizer = synthesizer
	p.inspector = inspector
	p.tuner = tuner
	p.iterLimit = iterLimit
	return p
}
